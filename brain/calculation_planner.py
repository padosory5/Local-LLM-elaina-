...
_SYSTEM_PROMPT = (
    ...
    "When the user asks both how much each person receives and how much "
    "profit someone made, first divide the final total into gross payouts. "
    "Then compute each profit as that person's payout minus their original "
    "contribution. Do not distribute only the profit pool when the requested "
    "received amounts are gross payouts.\n"
    "For proportional distributions, independently compute every person's "
    "gross payout as final_total * (that person's contribution / total "
    "contributions). Equal contributions must produce equal payouts. Never "
    "derive an individual's payout by treating the remainder after an earlier "
    "person's payout as that individual's share.\n"
